"""자식 프로세스 환경변수 sanitizer (Wave 4-E 도입, Wave 4-J 에서 ``shared/`` 로 중립화).

DevSecOps 분석기/검증기에서 외부 도구(``sonar-scanner`` / ``bandit`` /
``semgrep`` / ``pip-audit`` / ``flake8`` / sandbox ``pytest`` 등)를 호출할 때,
부모 프로세스의 환경변수가 무차별적으로 child 로 상속되면 ``ANTHROPIC_API_KEY``
``GITHUB_TOKEN`` ``AWS_SECRET_ACCESS_KEY`` ``DALLO_ENCRYPTION_KEY`` 같은 시크릿이
외부 도구의 로그/덤프/원격 텔레메트리 경로 또는 LLM 이 생성한 sandbox 코드로
흘러들 위험이 있다 (특히 AI/vibe-coding 컨텍스트에서 부모 셸에 다양한 토큰이
export 되어 있을 가능성이 높다).
...
Wave 4-J 메모: 본 모듈은 ``analyzer`` 또는 ``validator`` 패키지를 import 하지
않는다. 양쪽 모두에서 평등하게 의존할 수 있는 중립 boundary helper 다.
하위 호환을 위해 ``analyzer.command_env`` 가 본 함수를 re-export 하는 shim 으로
남는다.
"""
