...
- 본 wave 가 도입한 비공개(서비스 레이어) API: `api.services.red_blue_view.enrich_vulnerabilities(vulns) -> list[dict]` / `enrich_patches(patches, vulns) -> list[dict]` / `enrich_stats_payload(payload) -> dict` / `enrich_session_payload(payload) -> dict`. 네 함수 모두 dict in / dict out 의 순수 함수이며 `api/services/red_blue_view.py` 는 `shared.red_blue` 외 어떤 모듈도 import 하지 않는다 (FastAPI/Pydantic/`api.server`/DB/settings/env/filesystem/subprocess/network/time/LLM 의존 0).
...
- 클린 아키텍처 적합성: 본 wave 는 Dallo_v2 의 **router → service → shared helper** 3-레이어 분해를 그대로 유지하며, 라우터를 손대지 않고 enrichment 를 *서비스 레이어* 에 격리했다.
  ...
  - `api/services/dashboard_queries.py` — 기존 6 개 서비스 함수가 raw 데이터 조회 후 `red_blue_view.enrich_*` 호출로 끝단에서 키만 부착. 데이터 소스 분기/폴백 순서/입력 정규화는 기존 그대로.
