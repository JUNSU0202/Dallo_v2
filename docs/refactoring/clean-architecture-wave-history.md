...
> 본 문서는 Dallo DevSecOps 프로젝트가 **Wave 2-A 부터 Wave 4-Z 까지** 어떤 순서와 이유로 구조를 정리해 왔는지를 기록한다.
...
## 10. 현재 상태 (Wave 5-A 시점)

- Wave 5-A 는 **문서 전용(doc-only) wave** 다 (브랜치 `w5a-redblue-selection`, 구현 커밋 `55fa026 docs(refactoring): plan Red Blue backport selection`, 로컬 main 머지 커밋 `669ab31 merge: integrate Wave 5-A Red Blue backport selection docs`). 본 wave 의 산출물은 단 하나 — `docs/refactoring/redblue-backport-selection.md` 신규 작성 (Red/Blue 백포트 선별 및 아키텍처 계획 문서, 한국어, 초보자용 톤). 본 wave 는 운영 코드 / 테스트 / 패키지 / 설정 동작 / API / 프론트엔드 / 스키마 어떤 것도 변경하지 않는다. `shared/schemas.py` 무변경, `api/` / `agent/` / `analyzer/` / `validator/` / `db/` / `dashboard/` / `reports/` / `scripts/` / `tests/` / `.github/` / `config/` / dependency / lock / migration 모두 무변경. 본 문서 (`docs/refactoring/clean-architecture-wave-history.md`) 에 Wave 5-A 짧은 항목을 추가하는 변경만 동반된다.
...
- 마지막 검증된 결과 (Wave 5-A 시점, 문서 전용 wave 의 적정 검증 한정):
  - 본 wave 는 운영 코드 / 테스트 / 패키지 / 설정 / API / 프론트엔드 / 스키마 변경 0건이므로, Wave 4-Z post-state 의 ``pytest tests/ -q`` → **851 passed in 35.49s** 결과가 그대로 유효하며, 본 wave 가 그 카운트를 변경시키지 않는다. 본 wave 자체는 신규 회귀 테스트를 도입하지 않는다 (문서 전용 wave 의 정책). Wave 5-A 머지 직후 로컬 main 기준 full ``pytest tests/ -q`` → **851 passed in 36.71s** 로 재확인 (카운트 보존).
  - DOC 스모크: `python -c "<Wave 5-A 문서 토큰 검증>"` (본 디렉토리의 `docs/refactoring/redblue-backport-selection.md` 가 존재하고, 본문에 ``Gemini`` / ``gateway`` / ``Dallo_v2`` / ``Wave 5-A`` / ``red_blue`` / ``Semgrep`` / ``pytest`` 토큰을 모두 포함) → `DOC_SELECTION_SMOKE_PASS`. 본 스모크는 문서 본문의 핵심 키워드가 빠지지 않았음을 정적으로 검증한다.
  - `git status --short` / `git diff --stat main...HEAD` / `git diff --name-only main...HEAD` 가 본 wave 의 변경 범위를 `docs/refactoring/redblue-backport-selection.md` (신규) + `docs/refactoring/clean-architecture-wave-history.md` (본 항목 추가) 두 파일로만 제한함을 확인.
  - 본 wave 에는 코드 변경 wave 에 적용하는 매트릭스 (targeted/full pytest, API TestClient smoke, fake provider/runner/clock 강제, npm build, 보안 grep 가드, RED-first 증명, 독립 read-only 리뷰) 가 적용되지 않는다 — 적용 대상은 Wave 5-B 이후의 코드 변경 wave 다. 본 문서 (`docs/refactoring/redblue-backport-selection.md`) §9 가 그 매트릭스를 명문화한다.
- 본 wave 동안 push / PR / deploy / production DB / 실 외부 Dallo 호출 / 실 LLM 호출 / 실 GitHub API 호출 / 실 Redis 호출 / 실 pip-audit / 실 npm / 실 Bandit/Semgrep / 실 flake8 / 실 sandbox pytest / 실 외부 scanner / network 호출 모두 수행되지 않았다. 본 wave 는 `gusle01/main` 으로부터의 머지·체리픽도 수행하지 않았다 — read-only `git show gusle01/main:<path>` / `git diff $(git merge-base main gusle01/main)..gusle01/main -- <paths>` / `git merge-tree --write-tree main gusle01/main` 만 사용해 분류 근거를 수집했다.
- Rollback (Wave 5-A): 본 wave 는 로컬 ``main`` 으로 머지 커밋 `669ab31 merge: integrate Wave 5-A Red Blue backport selection docs` 로 통합되었으며, 구현 커밋은 `55fa026 docs(refactoring): plan Red Blue backport selection` 다. 표준 revert 경로는 `git revert -m 1 669ab31` 이다 (구현 커밋만 단독 rollback 이 필요한 경우의 보조 형태는 `git revert 55fa026`). revert 시 `docs/refactoring/redblue-backport-selection.md` 가 사라지고, `docs/refactoring/clean-architecture-wave-history.md` 의 본 §10 항목 + 문서 버전 푸터가 Wave 4-Z post-state 로 돌아간다. 본 wave 는 운영 코드 변경이 0 건이므로 revert 의 운영 영향도 0 이다. Wave 4-A ~ Wave 4-Z 의 모든 seam (`clock` / `file_io` / `runner` / `command_env` / dormant GitHub client) 은 본 wave 의 revert 와 무관하게 그대로 유지된다.
- 다음 후속 wave 후보 (어느 것도 아직 승인된 wave 가 아님 — 사용자 명시 승인이 매번 필요): **5-B** Red/Blue 순수 도메인/서비스 (`shared/red_blue.py`) → **5-C** `/api/red-blue/summary` 라우터/서비스 → **5-D** 대시보드 쿼리 응답 enrichment (additive only) → **5-E** `shared/llm_optimization.py` 순수 모듈 → **5-F** 파이프라인/API/Celery 옵션 plumbing (Gemini 디폴트 보존) → **5-G** Agent batch generation + clean audit (옵트인, fake provider) → **5-H** Heuristic fallback + 커스텀 Semgrep 룰 + multi-config (외부 레지스트리 금지) → **5-I** 프론트엔드 RedBlueView (gateway selector 미도입) → **5-J** 리포트 Red/Blue 섹션 + 문서 sync. 자세한 범위/검증/승인 게이트는 `docs/refactoring/redblue-backport-selection.md` §8 / §9 / §10 참조.
...
*문서 버전: Wave 5-A 시점 (2026-05-13). Wave 5-A 는 **문서 전용 wave** 다 — `docs/refactoring/redblue-backport-selection.md` 신규 작성 (Red/Blue 백포트 선별 및 아키텍처 계획, 한국어) + 본 문서에 §10 Wave 5-A 현재 상태 항목 추가의 두 변경으로만 구성된다. 운영 코드 / 테스트 / 패키지 / 설정 / API / 프론트엔드 / `shared/schemas.py` / DB / migration 변경 0건. Wave 4-Z 의 851 passed pytest baseline 은 Wave 5-A 머지 직후 로컬 main 기준 ``pytest tests/ -q`` → **851 passed in 36.71s** 로 보존 재확인. Wave 4-A ~ Wave 4-Z 의 모든 seam (`clock` / `file_io` / `runner` / `command_env` / dormant GitHub client) 은 본 wave 와 무관하게 그대로 유지된다. 본 wave 는 `gusle01/main` (HEAD `36041cc`, merge-base `ce5e869`) 의 Red/Blue 기능 의도만 분류하며, 직접 머지/체리픽은 (a) 5 개 파일 줄 단위 충돌과 (b) Wave 2~4-Z 의 클린 아키텍처 자산 삭제 위험 때문에 명시적으로 금지한다. gateway / claude-sonnet-4-6 디폴트는 사용자 정책 (Gemini / Google AI Studio 기본 경로 유지) 에 따라 **Reject** 으로 분류, `shared/schemas.py` Red/Blue 필드 추가는 별도 승인 없이 **Reject (default)** 로 분류. 후속 코드 wave (5-B ~ 5-J) 는 매번 사용자 명시 승인 + fake-seam 테스트 + revertable 단일 머지 커밋 단위로만 진행된다. 본 wave 동안 push / PR / deploy / 실 외부 호출 / `gusle01/main` 으로부터의 머지·체리픽 모두 수행되지 않았다. 로컬 main 머지 커밋은 `669ab31 merge: integrate Wave 5-A Red Blue backport selection docs` (구현 커밋 `55fa026 docs(refactoring): plan Red Blue backport selection`) 다.*
