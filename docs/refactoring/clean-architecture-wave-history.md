...
- 마지막 검증된 결과 (worktree 기준, pre-merge):
  - Pre-implementation RED (구현 diff 일시적 reverse + 신규 테스트 파일 유지): ``DALLO_ENCRYPTION_KEY=test-key DALLO_API_KEYS=test-api-key python -m pytest tests/test_quick_scan_clock_seam.py -q`` → exit 1, **7 failed, 2 passed, 9 errors in 0.50s** (`/tmp/dallo-wave4y-red-evidence.txt`). 실패 사유: baseline 에 ``set_clock`` / ``reset_clock`` / ``_clock`` 부재 + AST guard 가 두 endpoint 본체에서 직접 ``time.time()`` 호출 검출. 통과 2건은 인증 의존성 보존 가드 (본 wave 와 직교).
  ...
  - Targeted (신규 파일 + 인접 라우터 회귀): ``DALLO_ENCRYPTION_KEY=test-key DALLO_API_KEYS=test-api-key /home/ubuntu/dallo-devsecops/venv/bin/python -m pytest tests/test_quick_scan_clock_seam.py tests/test_api_server.py -q`` → **21 passed in 0.60s** — Wave 2-C 가 도입한 ``test_quick_scan_response_shape`` / ``test_quick_scan_requires_auth`` / ``test_quick_scan_project_response_shape`` 회귀 무영향 확인.
  - Full: ``DALLO_ENCRYPTION_KEY=test-key DALLO_API_KEYS=test-api-key /home/ubuntu/dallo-devsecops/venv/bin/python -m pytest tests/ -q`` → **838 passed in 35.46s** (Wave 4-X baseline 829 → +9 신규 회귀 테스트).
