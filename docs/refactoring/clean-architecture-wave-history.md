...
> 본 문서는 Dallo DevSecOps 프로젝트가 **Wave 2-A 부터 Wave 5-B 까지** 어떤 순서와 이유로 구조를 정리해 왔는지를 기록한다.
...
| **Wave 5 (A~B 시작, 5-A 는 문서 전용)** | Red/Blue 백포트 선별 + 첫 번째 Red/Blue 도메인 슬라이스 (`shared/red_blue.py` 순수 헬퍼) — 후속 5-C ~ 5-J 는 매번 별도 사용자 승인 wave | 원격 `gusle01/main` 의 Red/Blue 기능 의도만 분류 (직접 머지/체리픽 금지, backport-by-rewrite 합의), `shared/red_blue.py` 순수 함수 모듈 도입 (`enrich_vulnerability` / `enrich_patch` / `build_red_blue_summary` / `build_attack_paths` / `build_defense_comparison`) — FastAPI/DB/settings/time/file system 의존 0, `shared/schemas.py` 무변경, gateway / claude-sonnet-4-6 디폴트 거부 정책 유지, 905 passed | Red/Blue 백포트 정책을 문서화하고 첫 번째 안전한 도메인 슬라이스를 Dallo_v2 클린 아키텍처 위에 *다시 구현* — API/라우터/DB/대시보드/리포트/프로바이더 통합은 모두 후속 wave 로 명시적 deferral, Gemini / Google AI Studio 기본 경로 보존 |
...
- 본 wave 가 도입한 공개 API (모듈: `shared/red_blue.py`):
  - `enrich_vulnerability(vuln_dict) -> dict` — 취약점 dict 에 Red Team 공격 컨텍스트 (attack vector / CWE 템플릿 / impact 등) 를 파생 부착. 원본 dict 는 변형하지 않고 deepcopy 후 새 키를 추가한다.
  - `enrich_patch(patch_dict, vuln_dict | None) -> dict` — 패치 dict 에 Blue Team 방어 컨텍스트 (defense outcome / mitigation summary 등) 를 파생 부착. `vuln_dict` 가 주어지면 매칭 취약점 정보로 outcome 을 결정한다.
  - `build_red_blue_summary(vulns, patches) -> dict` — 취약점/패치 리스트로부터 Red/Blue 요약 (개수/심각도 분포/방어 성공 비율 등) 을 산출.
  - `build_attack_paths(vulns) -> list[dict]` — 취약점 리스트로부터 공격 경로 후보 목록을 결정적 순서로 생성.
  - `build_defense_comparison(vulns, patches) -> dict` — 같은 취약점에 대한 패치 적용 전/후 상태 비교 dict (Before / After / Delta) 를 산출.
- 본 wave 가 의식적으로 **하지 않은 것** (모두 후속 wave 로 명시적 deferral):
  - API 노출 0건 — 새 엔드포인트(예: `/api/red-blue/summary`) 도입은 Wave 5-C 의 범위다.
  - 라우터 / 서비스 / DTO 변경 0건 — `api/routers/red_blue.py` / `api/services/red_blue_summary.py` / `api/services/red_blue_view.py` / `api/dto/responses.py` 의 RedBlue 필드 어느 것도 본 wave 에는 없다.
  - DB / 대시보드 쿼리 변경 0건 — `db/models.py`, `db/service.py`, `api/services/dashboard_queries.py` 무변경.
  - 리포트 변경 0건 — `reports/report_generator.py` 의 Red/Blue 섹션 추가는 Wave 5-J 의 범위다.
  - 프로바이더/디폴트 모델 변경 0건 — `agent/provider_factory.py` 의 `_ACTIVE_PROVIDERS = {"gemini", "openrouter"}` 와 `api/routers/analyze.py` 의 `provider="gemini" / model="gemini-2.0-flash-lite"` 디폴트 보존. gateway / claude-sonnet-4-6 디폴트는 Wave 5-A §6 의 **Reject** 결정에 따라 본 wave 에도 도입되지 않는다.
  - `shared/schemas.py` 변경 0건 — Red/Blue 정보는 derived-only (서비스 enrichment 시점) 로 다루며 dataclass 계약에 박지 않는다.
- 클린 아키텍처 적합성: `shared/red_blue.py` 는 **순수 함수 모듈** (`shared/` 도메인 레이어) 이다. FastAPI / DB / settings / `time` / file system / subprocess / HTTP / requests 어디에도 의존하지 않으며, 입력은 dict, 출력은 dict, 부수효과 0, 입력 dict 변형 없음 (deepcopy 사용). 따라서 본 헬퍼는 라우터(`api/routers/`) / 서비스(`api/services/`) / 분석 파이프라인 어디서 호출되어도 안전하다 — 후속 5-C / 5-D 가 이 모듈을 *호출만* 하는 식으로 enrichment 를 부착한다.
- 마지막 검증된 결과 (Wave 5-B 시점):
  - Pre-merge worktree (branch `w5b-redblue-domain`):
    - Targeted: ``pytest tests/test_shared_red_blue.py -q`` → **54 passed**.
    - Full: ``pytest tests/ -q`` → **905 passed** (Wave 4-Z baseline 851 → +54 신규 회귀 테스트).
    - 독립 read-only 리뷰: **VERDICT: APPROVED**, `로컬 merge 승인 가능` (`/tmp/dallo-wave5b-independent-review.out.txt`).
  - Post-merge 로컬 ``main`` 기준 (head `761f974`):
    - Targeted: ``pytest tests/test_shared_red_blue.py -q`` → **54 passed in 0.13s**.
    - Full: ``pytest tests/ -q`` → **905 passed in 36.22s** (카운트/타이밍 보존 재확인).
    - 추가된 라인 (`shared/red_blue.py` + `tests/test_shared_red_blue.py`) 대상 secret-like / danger 호출 grep: clean (``(api_key|secret|password|token|passwd)\s*=\s*"…"`` / ``os.system(`` / ``shell=True`` / ``eval(`` / ``exec(`` / ``pickle.loads?(`` 0건).
    - `shared/red_blue.py` 금지 import/호출 빠른 스캔: clean (FastAPI / DB / `requests` / `subprocess` / `os.system` / `time` / file I/O 호출 부재).
    - 운영 추적 파일 대상 danger / secret assignment 스캔: clean (본 wave 의 추가 라인이 secret-like 리터럴 또는 금지 호출 패턴을 도입하지 않음).
- 본 wave 동안 push / PR / deploy / production DB / 실 외부 Dallo 호출 / 실 LLM 호출 / 실 GitHub API 호출 / 실 Redis 호출 / 실 pip-audit / 실 npm / 실 Bandit/Semgrep / 실 flake8 / 실 sandbox pytest / 실 외부 scanner / network 호출 모두 수행되지 않았다. 본 wave 는 `gusle01/main` 으로부터의 머지·체리픽도 수행하지 않았다 — Red/Blue 도메인 헬퍼는 Wave 5-A 가 합의한 backport-by-rewrite 정책에 따라 Dallo_v2 위에서 *처음부터 다시* 작성되었다.
- Rationale / Approval log / Independent review 산출물:
  - Rationale: `/tmp/dallo-wave5b-clean-architecture-rationale.md`.
  - 승인 로그: `/tmp/dallo-approval-log-wave5b.md`.
  - 독립 리뷰: `/tmp/dallo-wave5b-independent-review.out.txt`.
- Rollback (Wave 5-B): 본 wave 는 로컬 ``main`` 으로 머지 커밋 `761f974 merge: integrate Wave 5-B Red Blue domain helpers` 로 통합되었으며, 구현 커밋은 `6f8af7e feat(shared): add red blue domain helpers` 다. 표준 revert 경로는 `git revert -m 1 761f974` 이다 (구현 커밋만 단독 rollback 이 필요한 경우의 보조 형태는 `git revert 6f8af7e`). revert 시 `shared/red_blue.py` 와 `tests/test_shared_red_blue.py` 가 사라지고, 카운트는 Wave 5-A post-state (`pytest tests/ -q` → 851 passed) 로 돌아간다. 본 wave 는 운영 caller 가 0 (어떤 라우터/서비스/태스크/리포트도 아직 `shared.red_blue` 를 import 하지 않는다) 이므로 revert 의 운영 영향도 0 이다. Wave 2-A ~ Wave 5-A 의 모든 자산 (라우터/서비스/DTO 분리, `clock` / `file_io` / `runner` / `command_env` seam, dormant GitHub client seam, Wave 5-A 의 `docs/refactoring/redblue-backport-selection.md`) 은 본 wave 의 revert 와 무관하게 그대로 유지된다.
- 다음 권장 wave (아직 어떤 것도 승인되지 않음 — 매번 사용자 명시 승인 필요): **Wave 5-C** — `/api/red-blue/summary` 서비스/라우터 통합. 신규 `api/routers/red_blue.py` + `api/services/red_blue_summary.py` 도입, `api/server.py` 에 `include_router` 한 줄 추가, `shared.red_blue` 의 순수 헬퍼만 호출. **`shared/schemas.py` 무변경 (별도 승인 시에만 변경).** 인증 의존성은 기존 `Depends(verify_api_key)` 정책 그대로, DB → JSON → empty-shape 3 단 폴백 패턴은 `api.result_sources` 와 `db.service` 만 통해 구현. 자세한 범위/검증/승인 게이트는 `docs/refactoring/redblue-backport-selection.md` §5 #2 / §7-3 / §8 의 5-C 행 / §9 / §10 참조.
...
*문서 버전: Wave 5-B 시점 (2026-05-13). Wave 5-B 는 **Red/Blue 순수 도메인 헬퍼 마이크로 wave** 다 — Wave 5-A 가 합의한 backport-by-rewrite 정책의 첫 번째 안전한 슬라이스. `shared/red_blue.py` 신규 (순수 함수 모듈, FastAPI/DB/settings/time/file system 의존 0) + `tests/test_shared_red_blue.py` 신규 (54 신규 회귀 테스트) 의 두 변경으로 first-parent diff 가 한정된다. `shared/schemas.py` 무변경 (계약 보존), `api/` / `agent/` / `analyzer/` / `validator/` / `db/` / `dashboard/` / `reports/` / `scripts/` / `.github/` / `config/` / dependency / lock / migration 모두 무변경. Wave 4-Z 의 851 passed pytest baseline 위에 +54 신규 회귀 → 로컬 main 기준 ``pytest tests/ -q`` → **905 passed in 36.22s** 로 보존 재확인 (targeted ``tests/test_shared_red_blue.py -q`` → **54 passed in 0.13s**). 독립 read-only 리뷰 결과 **APPROVED** / `로컬 merge 승인 가능` (`/tmp/dallo-wave5b-independent-review.out.txt`). Wave 2-A ~ Wave 5-A 의 모든 자산 (라우터/서비스/DTO 분리, `clock` / `file_io` / `runner` / `command_env` seam, dormant GitHub client seam, `docs/refactoring/redblue-backport-selection.md`) 은 본 wave 와 무관하게 그대로 유지된다. 본 wave 는 `gusle01/main` 으로부터의 머지·체리픽을 수행하지 않으며 (Wave 5-A §4 의 직접 머지 금지 정책 그대로), Red/Blue 도메인 헬퍼는 Dallo_v2 위에서 처음부터 다시 작성되었다. Gemini / Google AI Studio 기본 경로 정책 보존 — gateway / claude-sonnet-4-6 디폴트는 Wave 5-A §6 의 **Reject** 결정에 따라 본 wave 에도 도입되지 않았다. API 노출 / 라우터 / 서비스 / DTO / DB / 대시보드 / 리포트 / 프로바이더/디폴트 모델 통합은 모두 후속 wave (5-C ~ 5-J) 로 명시적 deferral 이며, 각 wave 는 매번 사용자 명시 승인 + fake-seam 테스트 + revertable 단일 머지 커밋 단위로만 진행된다. 본 wave 동안 push / PR / deploy / production DB / 실 외부 Dallo 호출 / 실 LLM 호출 / 실 GitHub API 호출 / 실 Redis 호출 / 실 pip-audit / 실 npm / 실 Bandit/Semgrep / 실 flake8 / 실 sandbox pytest / 실 외부 scanner / network 호출 모두 수행되지 않았다. 로컬 main 머지 커밋은 `761f974 merge: integrate Wave 5-B Red Blue domain helpers` (구현 커밋 `6f8af7e feat(shared): add red blue domain helpers`) 다. 표준 revert 경로는 `git revert -m 1 761f974` (구현 커밋만 단독 rollback 시 `git revert 6f8af7e`) — 운영 caller 0 이므로 revert 의 운영 영향 0.*

*이전 문서 버전: Wave 4-Z 시점 (2026-05-12). Wave 4-Z 는 `analyzer/dependency_scanner.py` 의 ``DependencyScanner.__init__(runner=None, *, file_io=None)`` 에 keyword-only ``file_io`` 인자를 추가하고, ``scan_requirements_text`` / ``scan_package_json_text`` 의 임시 파일 쓰기 (``open(tmp_path, "w", encoding="utf-8")``) 를 ``self._io().write_text(...)`` 로, ``_fallback_pip_scan`` 의 requirements 라인 읽기 (``open(requirements_path, "r", encoding="utf-8").readlines()``) 를 ``self._io().read_text_lines(...)`` 로 위임한 마이크로 wave 다 (신규 회귀 13건, full 851 passed). ``file_io=None`` 분기로 ``analyzer.file_io.get_default_file_io()`` 가 use-time lazy 해석되므로 운영 동작 무변경 — ``DependencyScanner`` 생성자 시그니처(positional ``runner``) / ``scan_dependencies`` & ``scan_requirements_text`` & ``scan_package_json_text`` & ``_fallback_pip_scan`` 호출 시그니처 / ``DependencyVulnerability`` & ``DependencyScanResult`` 셰이프 / pip-audit & npm audit argv·cwd·timeout / Wave 4-H 의 ``DependencyCommandRunner`` runner seam / Wave 4-J 의 ``command_env`` boundary 와 child env sanitizer allowlists / 한국어 에러 문구 / fallback 파싱 정책 / API / DB / 스키마 모두 미수정. Wave 4-Y 의 quick scan ``_clock`` seam, Wave 4-X 의 ``execute_pipeline`` ``file_io`` seam, Wave 4-N 의 Bandit/Semgrep 파일 I/O 어댑터, Wave 4-O 의 validator 파일 쓰기 어댑터와 직교. `shared/schemas.py` 무변경. 로컬 main 머지 완료 (머지 커밋 `ad3c9a4`, 구현 커밋 `9085f5a`); 원격 push/PR/deploy/실 외부 호출 미수행.*
