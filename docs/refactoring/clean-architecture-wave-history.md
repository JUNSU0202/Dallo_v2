...
> 본 문서는 Dallo DevSecOps 프로젝트가 **Wave 2-A 부터 Wave 5-E 까지** 어떤 순서와 이유로 구조를 정리해 왔는지를 기록한다.
...
## 10. 현재 상태 (Wave 5-E 시점)

- Wave 5-E 는 **LLM 입력 최적화 순수 헬퍼 마이크로 wave** 다 (브랜치 `w5e-llm-optimization`, 구현 커밋 `379fbb5 feat(shared): add llm optimization helpers`, 로컬 main 머지 커밋 `a8fd274 merge: integrate Wave 5-E LLM optimization helpers`). Wave 5-A 가 `docs/refactoring/redblue-backport-selection.md` §5 #6 / §7-1 / §8 의 5-E 행 / §10 에서 합의한 **backport-by-rewrite** 정책의 두 번째 슬라이스이며, Red/Blue 도메인이 아닌 **non-Red/Blue Gusle01 기능 의도** (LLM 입력 최적화 — `cve_scope` / `cwe_scope` / `rule_scope` / `max_targets` / `max_context_chars` 등) 중 비교 끝에 유용하다고 판단된 부분을 Dallo_v2 클린 아키텍처 위에서 *다시 구현* 했다. 원격 `gusle01/main` 의 `shared/llm_optimization.py` 및 그 caller (`analyzer/pipeline.py` 옵션, `config/config.yaml` `llm.optimization`, `AnalyzeView.jsx`) 코드를 직접 머지/체리픽하지 않고, **Dallo_v2 의 `shared/` 도메인 레이어 위에서 순수 함수로 새로 작성** 했다. 본 wave 의 작업 트리는 first-parent 기준으로 `shared/llm_optimization.py` 신규 1건 + `tests/test_shared_llm_optimization.py` 신규 1건으로 한정된다. `shared/schemas.py` 무변경 (계약 보존), `api/` / `agent/` / `analyzer/` / `validator/` / `db/` / `dashboard/` / `reports/` / `scripts/` / `.github/` / `config/` / dependency / lock / migration 모두 무변경. Wave 5-B 의 `shared/red_blue.py` 또한 본 wave 의 변경과 무관하게 그대로 유지된다.
- 본 wave 가 도입한 공개 API (모듈: `shared/llm_optimization.py`):
  - `LLMOptimizationConfig` — 데이터클래스. 필드: `enabled` (필터/정렬/cap 우회 토글; trim 과 deepcopy 는 prompt 안정성을 위해 항상 적용), `cve_scope` (CVE 문자열 목록; vuln 의 텍스트 필드 검색), `cwe_scope` (CWE 토큰 또는 alias 목록; `SCOPE_ALIASES` 로 정규화 후 `vuln.cwe_id` 와 비교), `rule_scope` (분석기 rule id 대소문자 무시 목록), `max_targets` (선택 개수 상한; `<= 0` 이면 cap 미적용), `max_context_chars` (`function_code` / `code_snippet` / `file_imports` 글자수 상한; `<= 0` 이면 trim 미적용), `batch_enabled`, `batch_size`.
  - `SCOPE_ALIASES: dict[str, str]` — 도메인 직관(예: `SQLI` → `CWE-89`, `COMMAND_INJECTION` → `CWE-78`, `XSS` → `CWE-79`, `AUTH_BYPASS` / `AUTHENTICATION_BYPASS` → `CWE-288`, `PATH_TRAVERSAL` → `CWE-22`, `HARDCODED_SECRET` → `CWE-798`) alias 사전.
  - `trim_vulnerability_context(vulnerability, max_chars)` — deepcopy 후 `function_code` / `code_snippet` / `file_imports` 가 `max_chars` 초과 시 head + 마커 + tail 형태로 축약. `max_chars <= 0` 이면 trim 미적용 (deepcopy 만 적용).
  - `scoped_copy(vulnerability, max_chars)` — `trim_vulnerability_context` 의 공개 alias.
  - `optimize_llm_targets(vulnerabilities, config) -> (targets, summary)` — 입력 시퀀스를 deepcopy 보호 후 scope 필터링 → 우선순위 정렬 (`risk_level` → `severity` → `-cvss_score` → `file_path` → `line_number`) → `max_targets` cap → `max_context_chars` trim 의 순서로 수행. `summary` 는 JSON 호환 dict 로 후속 API 응답에 그대로 실릴 수 있는 형태 (`enabled`/`input_count`/`selected_count`/`cap_applied`/`max_targets`/`max_context_chars`/`batch_enabled`/`batch_size`/`scope.{cve,cwe,rule,aliases_used}`). 입력은 `shared.schemas.VulnerabilityReport` 또는 동일한 attribute set 을 갖는 임의 객체 (dataclass / 일반 클래스) — 본 모듈은 attribute 만 읽고 구체 타입에 의존하지 않는다.
  - 안전 가드 (Hermes follow-up): 사용자가 의미 있는 scope 토큰을 제공했지만 모든 카테고리 (`cwe_scope` / `rule_scope` / `cve_scope`) 에서 정규화/해석 결과 매칭 가능한 항목이 하나도 남지 않을 때만 전체로 넓히지 않고 0 으로 좁힌다. 단 한 카테고리라도 정상 해석되면 그 카테고리로 필터링하고 다른 카테고리의 알 수 없는 토큰은 무시한다 — 한쪽의 오타가 정상 scope 까지 무효화하지 않도록.
- 본 wave 가 의식적으로 **하지 않은 것** (모두 후속 wave 로 명시적 deferral):
  - `analyzer/pipeline.py::execute_pipeline` 옵션 추가 0건 — `*, clock=None, file_io=None` 뒤로 `cve_scope` / `cwe_scope` / `rule_scope` / `max_llm_targets` / `max_context_chars` / `batch_llm` / `llm_audit_when_clean` 의 keyword-only plumbing 은 Wave 5-F 의 범위다.
  - `api/routers/analyze.py::AnalyzeRequest` 필드 추가 0건 — 새 7 개 옵션 필드 도입은 Wave 5-F 의 범위다.
  - `api/services/analysis_pipeline.py::execute_analysis_job` / `api/tasks.py::run_analysis_task` 시그니처 변경 0건 — 옵션 dict 전달도 Wave 5-F 의 범위다.
  - `agent/llm_agent.py` 호출 0건 — 본 헬퍼는 LLM 제공자/모델 식별자/라우팅 정책에 무관하며, agent 측 batch / clean audit 변경은 Wave 5-G 의 범위다.
  - `config/config.yaml` `llm.optimization` 블록 도입 0건 — config 표면 결정은 5-F 와 함께 또는 별도 정책 wave 에서 다룬다.
  - `dashboard/src/` 변경 0건 — Analyze UI 의 LLM 최적화 옵션 노출은 Wave 5-I 의 범위다.
  - 리포트 변경 0건 — `reports/report_generator.py` 무변경.
  - 프로바이더/디폴트 모델 변경 0건 — `agent/provider_factory.py` 의 `_ACTIVE_PROVIDERS = {"gemini", "openrouter"}` 와 `api/routers/analyze.py` 의 `provider="gemini" / model="gemini-2.0-flash-lite"` 디폴트 보존. gateway / claude-sonnet-4-6 디폴트는 Wave 5-A §6 의 **Reject** 결정에 따라 본 wave 에도 도입되지 않는다 (본 모듈은 LLM 제공자 식별자를 알지도 못한다).
  - `shared/schemas.py` 변경 0건 — `VulnerabilityReport` dataclass 계약은 손대지 않으며, 본 헬퍼는 attribute 만 읽는다.
  - Wave 5-C / Wave 5-D 미진행 — `/api/red-blue/summary` 서비스/라우터 통합 (5-C) 및 대시보드 쿼리 응답 enrichment (5-D) 는 본 wave 이후로도 *시작되지 않은* 상태다. 본 wave 는 두 wave 와 직교한다 (`shared/red_blue.py` 와 `shared/llm_optimization.py` 는 서로 import 하지 않는다).
- 클린 아키텍처 적합성: `shared/llm_optimization.py` 는 **순수 함수 + 데이터클래스 모듈** (`shared/` 도메인 레이어) 이다. FastAPI / DB / settings / `time` / file system / subprocess / network / agent / validator / dashboard 어디에도 의존하지 않으며, 입력 시퀀스는 deepcopy 로 보호되어 caller-owned 객체가 변형되지 않는다. 임포트 시점에 부수효과 0. LLM 제공자/모델 식별자/라우팅 정책 토큰은 본 모듈의 관심사가 아니다. 따라서 본 헬퍼는 분석 파이프라인(`analyzer/pipeline.py`) / 서비스(`api/services/`) / 라우터(`api/routers/`) / Celery 태스크(`api/tasks.py`) 어디서 호출되어도 안전하다 — 후속 5-F 가 이 모듈을 *호출만* 하는 식으로 옵션 plumbing 을 부착한다.
- 마지막 검증된 결과 (Wave 5-E 시점):
  - Pre-merge worktree (branch `w5e-llm-optimization`):
    - Targeted: ``pytest tests/test_shared_llm_optimization.py -q`` → **65 passed in 0.09s**.
    - Full: ``pytest tests/ -q`` → **970 passed in 35.26s** (Wave 5-B baseline 905 → +65 신규 회귀 테스트).
    - Scope safety smoke: ``WAVE5E_SCOPE_SAFETY_SMOKE_PASS`` — Hermes follow-up 의 안전 가드 (모든 카테고리 미해석 → 0, 한 카테고리 해석 시 sibling 카테고리의 알 수 없는 토큰 무시) 가 실제로 의도대로 동작함을 worktree 에서 확인.
    - 독립 read-only 리뷰: **VERDICT: APPROVED**, `로컬 merge 승인 가능` (`/tmp/dallo-wave5e-independent-review.out.txt`).
  - Post-merge 로컬 ``main`` 기준 (head `a8fd274`):
    - Targeted: ``pytest tests/test_shared_llm_optimization.py -q`` → **65 passed in 0.18s**.
    - Full: ``pytest tests/ -q`` → **970 passed in 36.72s** (카운트/타이밍 보존 재확인).
    - 운영 helper (``shared/llm_optimization.py``) 금지 import/호출 빠른 스캔: clean (FastAPI / DB / `requests` / `subprocess` / `os.system` / `time` / file I/O 호출 부재).
    - 운영 helper 정책 토큰 스캔: clean (``"gateway"`` / ``"claude-sonnet-4-6"`` / ``LLM_PRIMARY_PROVIDER=gateway`` 등 Wave 5-A §6 가 거부한 토큰 0건).
    - 운영 추적 파일 대상 danger / secret assignment 스캔: clean.
    - 추가된 라인에서 SQL formatting 패턴 매칭은 오직 **테스트 fixture 문자열** 에만 등장하며 운영 코드 위험으로 이어지지 않음을 확인 (테스트 fixture 는 LLM optimization scope 매칭의 입력 조합을 만들기 위한 더미 데이터).
    - ``shared/schemas.py`` diff → **0** (계약 보존 재확인).
    - ``git status --short`` → clean (worktree 잔류 변경 0).
- 본 wave 동안 push / PR / deploy / production DB / 실 외부 Dallo 호출 / 실 LLM 호출 / 실 GitHub API 호출 / 실 Redis 호출 / 실 pip-audit / 실 npm / 실 Bandit/Semgrep / 실 flake8 / 실 sandbox pytest / 실 외부 scanner / network 호출 모두 수행되지 않았다. 본 wave 는 `gusle01/main` 으로부터의 머지·체리픽도 수행하지 않았다 — LLM 입력 최적화 헬퍼는 Wave 5-A 가 합의한 backport-by-rewrite 정책에 따라 Dallo_v2 위에서 *처음부터 다시* 작성되었다.
- 사용자 정책 / 프로세스 (다음 wave 에도 적용):
  - **Dallo_v2 의 리팩토링된 아키텍처가 Gusle01 비교 시 진실의 원천이다.** Gusle01 에서 발견된 *유용한 동작* 은 그것이 Dallo_v2 의 품질을 개선하거나 보존하는 경우에만, **기능 평행(feature parity)** 을 목표로 다시 구현한다.
  - 옛 모놀리스 코드를 **직접 머지/체리픽하지 않는다** — 본 wave 도 backport-by-rewrite 만 사용했다.
- Rationale / Approval log / Independent review 산출물:
  - Rationale: `/tmp/dallo-wave5e-clean-architecture-rationale.md`.
  - 승인 로그: `/tmp/dallo-approval-log-wave5e.md`.
  - 독립 리뷰: `/tmp/dallo-wave5e-independent-review.out.txt`.
- Rollback (Wave 5-E): 본 wave 는 로컬 ``main`` 으로 머지 커밋 `a8fd274 merge: integrate Wave 5-E LLM optimization helpers` 로 통합되었으며, 구현 커밋은 `379fbb5 feat(shared): add llm optimization helpers` 다. 표준 revert 경로는 `git revert -m 1 a8fd274` 이다. revert 시 `shared/llm_optimization.py` 와 `tests/test_shared_llm_optimization.py` 가 사라지고, 카운트는 Wave 5-B post-state (`pytest tests/ -q` → 905 passed) 로 돌아간다. 본 wave 는 운영 caller 가 0 (어떤 라우터/서비스/태스크/파이프라인/agent/리포트도 아직 `shared.llm_optimization` 을 import 하지 않는다) 이므로 revert 의 운영 영향도 0 이다. Wave 2-A ~ Wave 5-B 의 모든 자산 (라우터/서비스/DTO 분리, `clock` / `file_io` / `runner` / `command_env` seam, dormant GitHub client seam, Wave 5-A 의 `docs/refactoring/redblue-backport-selection.md`, Wave 5-B 의 `shared/red_blue.py`) 은 본 wave 의 revert 와 무관하게 그대로 유지된다.
- 다음 권장 wave (아직 어떤 것도 승인되지 않음 — 매번 사용자 명시 승인 필요):
  ...
  - **Wave 5-C** — `/api/red-blue/summary` 서비스/라우터 통합. 신규 `api/routers/red_blue.py` + `api/services/red_blue_summary.py` 도입, `api/server.py` 에 `include_router` 한 줄 추가, `shared.red_blue` 의 순수 헬퍼만 호출. **`shared/schemas.py` 무변경 (별도 승인 시에만 변경).** 인증 의존성은 기존 `Depends(verify_api_key)` 정책 그대로.
