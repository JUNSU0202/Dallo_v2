...
> 본 문서는 Dallo DevSecOps 프로젝트가 **Wave 2-A 부터 Wave 4-Y 까지** 어떤 순서와 이유로 구조를 정리해 왔는지를 기록한다.
...
- 작업 트리는 `api/routers/quick_scan.py` 수정 1건 + `tests/test_quick_scan_clock_seam.py` 신규 1건 (9개 신규 회귀 테스트) + `docs/refactoring/clean-architecture-wave-history.md` 갱신 1건으로 한정된다. `shared/schemas.py` 무변경, `analyzer/` / `agent/` / `validator/` / `db/` / `scripts/` / `.github/` / 다른 라우터(`api/routers/dashboard.py` / `analyze.py` / `report.py` / `dependencies.py` / `patch.py`) / dependency / lock / migration 무변경.
- 마지막 검증된 결과 (post-merge ``main`` 기준):
  - Pre-implementation RED (worktree 기준, 구현 diff 일시적 reverse + 신규 테스트 파일 유지): ``DALLO_ENCRYPTION_KEY=test-key DALLO_API_KEYS=test-api-key python -m pytest tests/test_quick_scan_clock_seam.py -q`` → exit 1, **7 failed, 2 passed, 9 errors in 0.50s** (`/tmp/dallo-wave4y-red-evidence.txt`). 실패 사유: baseline 에 ``set_clock`` / ``reset_clock`` / ``_clock`` 부재 + AST guard 가 두 endpoint 본체에서 직접 ``time.time()`` 호출 검출. 통과 2건은 인증 의존성 보존 가드 (본 wave 와 직교).
  - Syntax check: ``/home/ubuntu/dallo-devsecops/venv/bin/python -m py_compile tests/test_quick_scan_clock_seam.py api/routers/quick_scan.py`` → passed.
  - Targeted (worktree 기준, 신규 파일 + 인접 라우터 회귀): ``DALLO_ENCRYPTION_KEY=test-key DALLO_API_KEYS=test-api-key /home/ubuntu/dallo-devsecops/venv/bin/python -m pytest tests/test_quick_scan_clock_seam.py tests/test_api_server.py -q`` → **21 passed in 0.60s** — Wave 2-C 가 도입한 ``test_quick_scan_response_shape`` / ``test_quick_scan_requires_auth`` / ``test_quick_scan_project_response_shape`` 회귀 무영향 확인. post-merge ``main`` 에서의 targeted 재확인도 동일하게 **21 passed in 0.59s**.
  - Full (post-merge ``main`` 기준): ``DALLO_ENCRYPTION_KEY=test-key DALLO_API_KEYS=test-api-key /home/ubuntu/dallo-devsecops/venv/bin/python -m pytest tests/ -q`` → **838 passed in 33.91s** (Wave 4-X baseline 829 → +9 신규 회귀 테스트). 참고: pre-merge worktree 측정치는 `838 passed in 35.46s` 였으며, post-merge main 재측정에서 33.91s 로 갱신됨 — 카운트(838) 와 +9 신규 회귀는 동일하게 보존.
  - 정적 안전 grep: 운영 코드 추가분 (``api/routers/quick_scan.py``) 의 추가 라인에 secret-like / danger 호출 0건. 두 endpoint 본체 AST 스캔에서 ``time.time()`` 직접 호출 0건 (정적 가드 — ``test_quick_scan_endpoint_body_has_no_direct_time_time_call`` / ``test_quick_scan_project_endpoint_body_has_no_direct_time_time_call`` 가 회귀 차단). ``shared/schemas.py`` diff empty (무변경 보존). 변경 파일 대상 시크릿 리터럴 스캔 ok (Hermes 확인).
- 본 wave 동안 push / PR / deploy / production DB / 실 외부 Dallo 호출 / 실 LLM 호출 / 실 GitHub API 호출 / 실 Redis 호출 / 실 Bandit/Semgrep / 실 flake8 / 실 sandbox pytest / 실 외부 scanner / network 호출 모두 수행되지 않았다. 테스트는 FastAPI ``TestClient`` + fake clock 만 사용한다.
- Rollback (Wave 4-Y): 본 wave 는 로컬 ``main`` 으로 머지 커밋 `76a2a86 merge: integrate Wave 4-Y quick scan clock seam` 로 통합되었으며, 구현 커밋은 `8222777 refactor(api): add Wave 4-Y quick scan clock seam` 다. 표준 revert 경로는 `git revert -m 1 76a2a86` 이다 (구현 커밋만 단독 rollback 이 필요한 경우의 보조 형태는 `git revert 8222777`). revert 시 `api/routers/quick_scan.py` 가 Wave 4-X post-state (``_clock`` / ``set_clock`` / ``reset_clock`` / ``_elapsed_ms`` 없이 두 endpoint 본체가 inline ``time.time()`` 두 호출을 직접 수행) 로 돌아가고, ``tests/test_quick_scan_clock_seam.py`` 가 사라진다. 운영 caller (HTTP client / 프론트엔드) 가 새 seam 을 모르고도 정상 호출하므로 revert 의 운영 영향 0. Wave 4-X 의 ``execute_pipeline`` ``file_io`` seam, Wave 4-W 의 ``LLMCache`` ``clock`` seam, Wave 4-V 의 ``execute_pipeline`` ``clock`` seam, Wave 4-U 의 ``_build_result`` ``now`` seam, Wave 4-T 의 analysis pipeline service clock seam, Wave 4-P 의 DB ``DateTime`` default clock seam 은 본 wave 의 revert 와 무관하게 그대로 유지된다.
...
*문서 버전: Wave 4-X 시점 (2026-05-11). Wave 4-X 는 `analyzer/file_io.py::FileIO` 에 ``write_text(path, content)`` 메서드를 추가하고, `analyzer/pipeline.py::execute_pipeline` 에 keyword-only ``file_io=None`` fakeable 파일 I/O seam 을 도입한 마이크로 wave 다 (``TestExecutePipelineFileIOSeam`` 6 케이스 신규, full 829 passed). default ``None`` 분기로 ``get_default_file_io()`` 가 lazy 해석되므로 운영 동작 무변경 — ``execute_pipeline`` public 시그니처 / ``PipelineResult`` 셰이프 / ``MAX_CODE_SIZE`` 검증 / 한국어 에러 문구 / 임시 디렉토리/cleanup 정책 / API / DB / 스키마 모두 미수정. Wave 4-V 의 ``clock`` seam, Wave 4-U 의 ``_build_result`` ``now`` seam, Wave 4-N 의 ``write_json`` / ``read_text_lines`` 와 직교. `shared/schemas.py` 무변경. 로컬 main 머지 완료 (머지 커밋 `fde2b1d`, 구현 커밋 `429dda6`); 원격 push/PR/deploy/실 외부 호출 미수행.*
