...
> 본 문서는 Dallo DevSecOps 프로젝트가 **Wave 2-A 부터 Wave 4-X 까지** 어떤 순서와 이유로 구조를 정리해 왔는지를 기록한다.
...
- 작업 트리는 `analyzer/file_io.py` 수정 1건 + `analyzer/pipeline.py` 수정 1건 + `tests/test_pipeline_integration.py` 갱신 1건 (``TestExecutePipelineFileIOSeam`` 6개 신규 테스트 추가) + `docs/refactoring/clean-architecture-wave-history.md` 갱신 1건으로 한정된다. `shared/schemas.py` 무변경, `api/` / `agent/` / `validator/` / `db/` / `scripts/` / `.github/` / dependency / lock / migration 무변경.
- 마지막 검증된 결과 (post-merge ``main`` 기준):
  - Targeted (class): `pytest tests/test_pipeline_integration.py::TestExecutePipelineFileIOSeam -q` → **6 passed**.
  - Broader (file I/O seam 패밀리): `pytest tests/test_pipeline_integration.py tests/test_bandit_file_io_seam.py tests/test_semgrep_file_io_seam.py -q` → **44 passed**.
  - Full: `pytest tests/ -q` → **829 passed in 32.79s** (Wave 4-W baseline 823 → +6 신규 회귀 테스트). 참고: pre-merge worktree 측정치는 `829 passed in 33.48s` 였으며, post-merge main 재측정에서 32.79s 로 갱신됨.
  - Optional caller regression: `pytest tests/test_api_analysis_pipeline_service.py tests/test_api_analyze_router.py -q` → **44 passed**.
  - ``git diff --check`` clean — whitespace 회귀 없음.
  - ``python -m py_compile analyzer/file_io.py analyzer/pipeline.py tests/test_pipeline_integration.py`` ok — 신규 코드/테스트 syntax 회귀 없음.
  - 독립 read-only 리뷰: **REVIEW_VERDICT: APPROVE** — blocker 0건, seam 우회 0건, ``FileIO`` 어댑터 단일 책임 보존 확인, ``execute_pipeline`` public 시그니처/``PipelineResult`` 셰이프/한국어 에러 문구 불변 확인.
  - 정적 안전 grep: 운영 코드 추가분 (``analyzer/file_io.py``, ``analyzer/pipeline.py``) 의 추가 라인에 secret-like / danger 호출 0건. 참고: 전체 diff (문서 포함) 추가 라인 grep 은 본 문서의 금지어 설명 문장 자체에 self-match 1 건이 잡히지만 이는 실제 코드 위험이 아니라 설명 문장 인용으로 인한 매칭이다. ``execute_pipeline`` 본체 AST 스캔에서 ``open(..., 'w')`` 직접 호출 0건 (정적 가드).
...
- Rollback (Wave 4-X): 본 wave 는 로컬 ``main`` 으로 머지 커밋 `fde2b1d merge: integrate Wave 4-X analyzer pipeline file io seam` 로 통합되었으며, 구현 커밋은 `429dda6 refactor(analyzer): add Wave 4-X pipeline file io seam` 다. 표준 revert 경로는 `git revert -m 1 fde2b1d` 다 (구현 커밋만 단독 rollback 이 필요한 경우의 보조 형태는 `git revert 429dda6`). revert 시 `analyzer/pipeline.py::execute_pipeline` 가 Wave 4-W post-state (``file_io`` 인자 없이 inline ``with open(file_path, "w", encoding="utf-8") as f: f.write(code)``) 로 돌아가고, ``FileIO.write_text`` 메서드가 제거되며, ``tests/test_pipeline_integration.py`` 의 ``TestExecutePipelineFileIOSeam`` 클래스가 사라진다. 운영 caller (라우터/태스크/Dashboard 업로드 경로) 가 새 ``file_io`` 인자를 쓰지 않으므로 revert 의 운영 영향 0. Wave 4-W 의 ``LLMCache`` ``clock`` seam, Wave 4-V 의 ``execute_pipeline`` ``clock`` seam, Wave 4-U 의 ``_build_result`` ``now`` seam, Wave 4-N 의 ``FileIO`` ``write_json`` / ``read_text_lines`` 는 본 wave 의 revert 와 무관하게 그대로 유지된다.
