...
- **Wave 5-E** — `shared/llm_optimization.py` 순수 모듈: **완료 (로컬 main 통합)**. 본 wave 는 §8 의 권장 순서 (5-B → 5-C → 5-D → 5-E) 와 달리 5-C / 5-D 보다 먼저 진행되었다 — Red/Blue 도메인이 아닌 **non-Red/Blue Gusle01 기능 의도** (LLM 입력 최적화) 중 비교 끝에 유용하다고 판단된 부분을 우선 backport-by-rewrite 한 결정에 따른다. 본 wave 와 Wave 5-C / 5-D 는 직교한다 (`shared/red_blue.py` 와 `shared/llm_optimization.py` 는 서로 import 하지 않는다).
  - 브랜치: `w5e-llm-optimization`. 구현 커밋 `379fbb5 feat(shared): add llm optimization helpers`. 로컬 main 머지 커밋 `a8fd274 merge: integrate Wave 5-E LLM optimization helpers`.
  - 범위 (first-parent diff): `shared/llm_optimization.py` 신규 1건 + `tests/test_shared_llm_optimization.py` 신규 1건. `shared/schemas.py` 무변경 (§6-2 / §10 정책 보존). `analyzer/pipeline.py` / `api/routers/analyze.py::AnalyzeRequest` / `api/services/analysis_pipeline.py` / `api/tasks.py` / `agent/llm_agent.py` / `config/config.yaml` / 대시보드 / 리포트 통합 0건 (모두 Wave 5-F 이후로 명시적 deferral) — 본 wave 는 §8 의 5-E 행이 명시한 *호출 없음* 의 순수 모듈 슬라이스로 한정된다.
  - 공개 API: `LLMOptimizationConfig` 데이터클래스, `SCOPE_ALIASES` 사전 (`SQLI` / `SQL_INJECTION` / `CMDI` / `COMMAND_INJECTION` / `XSS` / `AUTH_BYPASS` / `AUTHENTICATION_BYPASS` / `PATH_TRAVERSAL` / `HARDCODED_SECRET` → CWE 정규화), `trim_vulnerability_context(vulnerability, max_chars)` / `scoped_copy(vulnerability, max_chars)`, `optimize_llm_targets(vulnerabilities, config) -> (targets, summary)`. 입력은 `VulnerabilityReport` 또는 동등 attribute set 의 임의 객체, 출력 `summary` 는 JSON 호환 dict. 안전 가드 (Hermes follow-up): 모든 카테고리 미해석 시에만 0 으로 좁힘, 한 카테고리 해석 시 sibling 카테고리의 알 수 없는 토큰은 무시.
  - 검증: targeted `pytest tests/test_shared_llm_optimization.py -q` → 65 passed (worktree 0.09s / post-merge main 0.18s), full `pytest tests/ -q` → 970 passed (Wave 5-B baseline 905 → +65; worktree 35.26s / post-merge main 36.72s), pre-merge scope safety smoke `WAVE5E_SCOPE_SAFETY_SMOKE_PASS`, 독립 read-only 리뷰 결과 APPROVED, 운영 helper 금지 import/호출 + 정책 토큰 + danger/secret 스캔 모두 clean, `shared/schemas.py` diff 0, `git status --short` clean.
  - 정책 보존: Gemini / Google AI Studio 기본 경로 유지, gateway / claude-sonnet-4-6 디폴트 미도입 (본 모듈은 LLM 제공자/모델 식별자/라우팅 정책 토큰에 무관). `shared/schemas.py` 무변경.
  - Rollback: `git revert -m 1 a8fd274`. 운영 caller 0 이므로 revert 의 운영 영향 0.
  - 자세한 wave 상세는 `docs/refactoring/clean-architecture-wave-history.md` §10 (현재 상태, Wave 5-E 시점) 및 §5 의 5-E 행 참조.
- **Wave 5-C** — `/api/red-blue/summary` 서비스/라우터 통합: **완료 (로컬 main 통합)**. 본 wave 는 §5 #2 / §7-3 / §8 의 5-C 행 합의에 따라 진행되었다 — Wave 5-B 가 도입한 `shared.red_blue.build_red_blue_summary` 순수 헬퍼를 Dallo_v2 의 router → service → shared helper 3-레이어 분해 위에서 인증된 API 엔드포인트로 노출한다.
  - 브랜치: `w5c-redblue-summary-api`. 구현 커밋 `e3fd51c feat(api): add red blue summary endpoint`. 로컬 main 머지 커밋 `7fe5c2d merge: integrate Wave 5-C Red Blue summary API`.
  - 범위 (first-parent diff): `api/dto/responses.py` 수정 1건 (신규 `RedBlueSummaryResponse`) + `api/routers/red_blue.py` 신규 1건 (얇은 HTTP 레이어, `Depends(verify_api_key)`) + `api/server.py` 수정 1건 (라우터 import/include 2줄) + `api/services/red_blue_summary.py` 신규 1건 (HTTP 비의존 — `fastapi`/`pydantic`/`api.server` import 0건, AST 가드로 정적 강제; DB → JSON → 빈 셰이프 3단 fail-closed 폴백, `_as_list` 입력 정규화) + `tests/test_api_red_blue_router.py` 신규 1건 (16 신규 회귀 테스트). `shared/schemas.py` 무변경 (§6-2 / §10 정책 보존, diff 0 bytes vs `a58728b`), `shared/red_blue.py` / `shared/llm_optimization.py` 무변경, `agent/` / `analyzer/` / `validator/` / `db/` / `dashboard/` / `reports/` / `scripts/` / `.github/` / `config/` / dependency / lock / migration 모두 무변경.
  - 공개 API: HTTP `GET /api/red-blue/summary` (인증, top-level keys `red_team`/`blue_team`/`comparison`/`attack_paths` 4개 동결, `_Permissive` + `response_model_exclude_unset=True`), Python `api.services.red_blue_summary.get_red_blue_summary(*, include_attack_paths=True) -> dict`. 폴백 순서: `db.service.get_latest_analysis()` → `api.result_sources.load_full_result()` → `shared.red_blue.build_red_blue_summary([], [])` 빈 셰이프. broken DB / 손상된 JSON 은 `_load_source` 의 `try/except Exception` 으로 차단되어 500 으로 새지 않음.
  - 검증: pre-implementation RED targeted `pytest tests/test_api_red_blue_router.py -q` → **16 failed in 0.71s** (ImportError + 라우트 부재), post-merge targeted (router+shared) → **70 passed in 0.73s**, adjacent API contract/server/lifespan/syspath → **46 passed in 4.07s**, full `pytest tests/ -q` → **986 passed in 37.73s** (Wave 5-E baseline 970 → +16 신규). FastAPI smoke `WAVE5C_ROUTE_SMOKE_PASS` / post-merge `WAVE5C_MAIN_ROUTE_SMOKE_PASS` (인증 200 / 미인증 401). 독립 read-only 리뷰 결과 APPROVED, blocking issues none (`/tmp/dallo-wave5c-independent-review.out.txt`). 변경 diff 의 secret-like 리터럴 / `os.system(` / `shell=True` / `eval(` / `exec(` / `pickle.loads?(` / 정책 토큰 (`gateway` / `claude-sonnet-4-6` / `LLM_PRIMARY_PROVIDER=gateway`) 모두 clean, 서비스 모듈 forbidden import AST clean.
  - 정책 보존: Gemini / Google AI Studio 기본 경로 유지, gateway / claude-sonnet-4-6 디폴트 미도입 (본 라우터/서비스/DTO 는 LLM 제공자 식별자를 알지도 못한다). 인증 정책은 기존 `Depends(verify_api_key)` (즉 `DALLO_API_KEYS`) 그대로. `shared/schemas.py` 무변경. 모든 테스트 더블은 pytest `monkeypatch` 만 사용.
  - Rollback: `git revert -m 1 7fe5c2d` (구현 커밋만 revert 시 `git revert e3fd51c`). 본 wave 는 read-only 조회 엔드포인트만 도입했고 운영 caller (대시보드 등) 가 아직 본 엔드포인트를 소비하지 않으므로 revert 의 운영 영향 0.
  - 자세한 wave 상세는 `docs/refactoring/clean-architecture-wave-history.md` §10 (현재 상태, Wave 5-C 시점) 및 §5 의 5-C 행 참조.
- **Wave 5-D** — 대시보드 쿼리 응답 additive Red/Blue enrichment: **완료 (로컬 main 통합)**. 본 wave 는 §5 #3 / §7-3 / §8 의 5-D 행 합의에 따라 진행되었다 — Wave 5-B 가 도입한 `shared.red_blue.enrich_vulnerability` / `enrich_patch` / `build_red_blue_summary` 순수 헬퍼를 Dallo_v2 의 `api/services/dashboard_queries.py` 끝단 + 신규 `api/services/red_blue_view.py` + DTO Optional 필드 추가 위에서 기존 대시보드 read 엔드포인트 6 개 응답에 *additive only* 로 부착한다.
  - 구현 커밋 `5926fb3 feat(api): add dashboard red blue enrichment`. 로컬 main 머지 커밋 `30ea1df merge: integrate Wave 5-D dashboard Red Blue enrichment`.
  - 범위 (first-parent diff): `api/dto/responses.py` 수정 1건 (`VulnerabilityItem` 에 Optional Red Team 키 7 개 — `red_team_phase`/`attack_vector`/`attack_scenario`/`security_impact`/`blue_team_strategy`/`exploitability`/`attack_plan`; `PatchItem` 에 Optional Blue Team 키 5 개 — `blue_team_phase`/`defense_strategy`/`defense_outcome`/`residual_risk`/`defense_plan`) + `api/services/dashboard_queries.py` 수정 1건 (4 개 서비스 함수 — `get_vulnerabilities` / `get_patches` / `get_stats` / `get_session_detail` — 끝단에 `red_blue_view.enrich_*` 직접 wiring; `/api/vulnerabilities/by-file` / `/api/vulnerabilities/by-type` 의 raw 집계 경로는 내부에서 `get_vulnerabilities()` 를 호출하므로 enriched vulnerability dict 를 간접 경로로 받으며 별도 wiring 사이트가 아니다) + `api/services/red_blue_view.py` 신규 1건 (HTTP/도메인 비의존 — `shared.red_blue` 외 import 0; FastAPI/Pydantic/`api.server`/DB/settings/env/filesystem/subprocess/network/time/LLM 의존 0; `enrich_vulnerabilities` / `enrich_patches` / `build_view_summary` / `enrich_stats` / `enrich_analysis_result`. patch enrichment 가 vulnerability 를 먼저 enrich 한 뒤 매칭하므로 CWE-89 매칭 patch 는 generic fallback 대신 구체적 `parameterized queries` defense strategy 를 받는다) + `tests/test_api_dashboard_red_blue_passthrough.py` 신규 1건 (26 신규 회귀). `api/routers/dashboard.py` 무변경 (얇은 라우터 유지), `shared/schemas.py` 무변경 (§6-2 / §10 정책 보존, diff 0 bytes vs `7fe5c2d`), `shared/red_blue.py` / `shared/llm_optimization.py` / `api/routers/red_blue.py` / `api/services/red_blue_summary.py` / `agent/` / `analyzer/` / `validator/` / `db/` / `dashboard/` (프론트엔드) / `reports/` / `scripts/` / `.github/` / `config/` / dependency / lock / migration 모두 무변경.
  - 응답 셰이프 (additive only — 기존 top-level/기존 키는 한 글자도 바뀌지 않는다): `/api/vulnerabilities` 는 top-level `count` / `vulnerabilities` 보존 + 각 item 에 Red Team 키 7 개 추가; `/api/patches` 는 top-level `count` / `patches` + 기존 patch 메타 키 보존 + 각 item 에 Blue Team 키 5 개 추가; `/api/stats` JSON 폴백은 raw `vulnerabilities` 또는 `patches` 가 존재할 때만 nested `red_blue_summary` 부착 (빈 폴백은 기존 6 키 셰이프 정확 보존); `/api/sessions/{session_id}` 는 미존재 세션의 `{"error": "Session not found"}` 보존 + 세션 detail dict 에 vulnerabilities/patches 가 있으면 enriched nested list + `red_blue_summary` 부착; `/api/vulnerabilities/by-file` / `/api/vulnerabilities/by-type` 는 집계 셰이프 그대로.
  - 검증: pre-implementation RED targeted `pytest tests/test_api_dashboard_red_blue_passthrough.py -q` → **19 failed, 7 passed** (`api.services.red_blue_view` 부재, `attack_plan` / `defense_plan` / `red_blue_summary` / session enrichment 부재). Hardening RED: 강화된 assertion 적용 후 `defense_strategy` 가 generic fallback 으로 떨어지던 2 건 실패 → 매칭 순서 수정 후 GREEN. Pre-merge worktree: targeted **26 passed in 0.53s**, adjacent (`tests/test_api_dashboard_queries_service.py tests/test_api_contract.py tests/test_api_red_blue_router.py tests/test_shared_red_blue.py -q`) **111 passed in 3.13s**, full `pytest tests/ -q` → **1012 passed in 34.31s** (Wave 5-C baseline 986 → +26 신규). FastAPI smoke `WAVE5D_WORKTREE_ROUTE_SMOKE_PASS`, 보안 스캔 `WAVE5D_WORKTREE_SECURITY_SCAN_PASS`, 독립 read-only 리뷰 결과 APPROVED / blocker 없음 (`/tmp/dallo-wave5d-independent-review.out.txt`). Post-merge 로컬 ``main`` (head `30ea1df`): targeted **26 passed in 0.59s**, adjacent **111 passed in 3.25s**, full **1012 passed in 35.88s**, FastAPI smoke `WAVE5D_MAIN_ROUTE_SMOKE_PASS`, 보안 스캔 `WAVE5D_MAIN_SECURITY_SCAN_PASS`, ``shared/schemas.py`` diff vs `7fe5c2d` → 0 bytes, ``git status --short`` → clean.
  - 정책 보존: Gemini / Google AI Studio 기본 경로 유지, gateway / claude-sonnet-4-6 디폴트 미도입 (본 view/서비스/DTO 는 LLM 제공자 식별자/라우팅 정책 토큰을 알지도 못한다). 인증 정책은 기존 `Depends(verify_api_key)` 그대로. `shared/schemas.py` 무변경. 모든 테스트 더블은 pytest `monkeypatch` 만 사용.
  - 의식적으로 deferred: 프론트엔드 RedBlueView/탭/카드 (Wave 5-I), 리포트 Red/Blue 섹션 (Wave 5-J), LLM 최적화 plumbing (Wave 5-F), agent batch+clean audit (Wave 5-G), heuristic fallback + Semgrep multi-config (Wave 5-H).
  - Rollback: `git revert -m 1 30ea1df` (구현 커밋만 revert 시 `git revert 5926fb3`). 응답 enrichment 는 additive only 이며 운영 caller (프론트엔드 등) 가 아직 Red/Blue 키를 소비하지 않으므로 revert 의 운영 영향 0.
  - 자세한 wave 상세는 `docs/refactoring/clean-architecture-wave-history.md` §10 (현재 상태, Wave 5-D 시점) 및 §5 의 5-D 행 참조.
- **Wave 5-F ~ Wave 5-J** — 아직 시작되지 않음. 각 wave 의 진입 조건은 본 문서 §8 / §9 / §10 의 승인 게이트 그대로다. 다음 권장 후보는 (a) **Wave 5-F** — Wave 5-E 가 도입한 `shared/llm_optimization.py` 를 `analyzer/pipeline.execute_pipeline` / `AnalyzeRequest` / Celery 태스크에 plumbing (디폴트는 기존 동작 보존), 또는 (b) 그 이후의 **Wave 5-I / 5-J** — 프론트엔드 RedBlueView / 리포트 Red/Blue 섹션 (Wave 5-D 가 출하한 응답 contract 가 입력). **Wave 5-D 는 이미 완료되었으므로 더 이상 다음 권장 후보가 아니다.** 두 후보 모두 별도 사용자 명시 승인 + fake-seam 테스트 + revertable 단일 머지 커밋이 필요하다.
