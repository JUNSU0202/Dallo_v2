...
- **Wave 5-D / Wave 5-F ~ Wave 5-J** — 아직 시작되지 않음. 각 wave 의 진입 조건은 본 문서 §8 / §9 / §10 의 승인 게이트 그대로다. 다음 권장 후보는 (a) **Wave 5-D** — 대시보드 쿼리 응답에 `shared.red_blue.enrich_*` 적용 (additive only, 기존 키 불변), 또는 (b) **Wave 5-F** — Wave 5-E 가 도입한 `shared/llm_optimization.py` 를 `analyzer/pipeline.execute_pipeline` / `AnalyzeRequest` / Celery 태스크에 plumbing (디폴트는 기존 동작 보존). 두 후보 모두 별도 사용자 명시 승인 + fake-seam 테스트 + revertable 단일 머지 커밋이 필요하다.
